...
import {
  calcInstitutional,
  diffInstitutional,
} from "../utils/institutionalAnalysis.js";
...
const DiffAlerts = React.memo(function DiffAlerts({ alerts }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 13 }}>🔔</span> WHAT CHANGED IN THE LAST 2
        MINUTES
      </div>
      {alerts.map((a, i) => (
        <div
          ...
          style={{
            ...
            padding: "7px 0",
            borderBottom:
              i < alerts.length - 1 ? "1px solid #ff7b0022" : "none",
          }}
        >
...
export const InstitutionalPanel = React.memo(function InstitutionalPanel({
  ...
  return (
    <div>
        ...
        {[
          ...
          return (
            ...
            >
              ...
              <div
                style={{ fontSize: 9, color: `${color}88`, marginBottom: 6 }}
              >
                {hint}
              </div>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {zones.length ? (
                  ...
                  <span style={{ color: C.muted, fontSize: 10 }}>
                    No {side === "PE" ? "support" : "resistance"} found
                  </span>
                )}
              </div>
              {zones.length > 0 && (
                <div
                  style={{ fontSize: 10, color: `${color}88`, marginTop: 6 }}
                >
                  Total size: {fmt(oiSum)} (
                  {((oiSum / (total || 1)) * 100).toFixed(1)}% of all{" "}
                  {side === "PE" ? "Put" : "Call"} positions)
                </div>
              )}
            </div>
          );
      ...
      <Card>
        <CardTitle icon="🔥">
          Biggest Position Changes (Where Big Money Moved)
        </CardTitle>
        {topSpikes.length === 0 ? (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: C.blue, marginTop: 2 }}>
                  ...
                  Positions added: {fmt(s.doi)} · Volume: {fmt(s.vol)} · Price:
                  ₹{s.ltp}
                  {s.chg !== undefined && (
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          {[
            {
              title: `Top Call positions — ${concCe.toFixed(1)}% of all Calls are here`,
              color: C.red,
              items: top3Ce,
              maxOI: maxCeOI,
              side: "CE",
            ...
            {
              title: `Top Put positions — ${concPe.toFixed(1)}% of all Puts are here`,
              color: C.green,
              items: top3Pe,
              maxOI: maxPeOI,
              side: "PE",
              ...
              {items.map((r, i) => (
                <div key={r.strikePrice} style={{ marginBottom: 6 }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${(r[side].openInterest / maxOI) * 100}%`,
                        height: "100%",
